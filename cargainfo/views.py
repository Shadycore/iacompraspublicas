...
from analizaproveedor.models import contract, sri_ente, award, release, planning, supplier, tender
...
def cargar_info_csv(csv_file, user, separador):
    ...
    with open(file=file.name, mode='r', encoding="utf-8") as f:
        ...
        for line in reader:
            ...
            contractPeriod_startDate = columns[7].replace("'","")[:10]
            ...
            dateSigned = columns[13].replace("'","")[:10]
...
def cargar_info_award(csv_file, user, separador):
    ...
    with open(file=file.name, mode='r', encoding="utf-8") as f:
        ...
        for line in reader:
            ...
            contractPeriod_startDate = columns[13].replace("'","")[:10]
            contractPeriod_endDate = columns[14].replace("'","")[:10]
            contractPeriod_maxExtentDate = columns[15].replace("'","")[:10]
...
def cargar_info_tender(csv_file, user, separador):
    ...
    with open(file=file.name, mode='r', encoding="utf-8") as f:
        ...
        for line in reader:
            ...
            tenderPeriod_startDate = columns[14].replace("'","")[:10]
            tenderPeriod_endDate = columns[15].replace("'","")[:10]
            tenderPeriod_maxExtentDate = columns[16].replace("'","")
            tenderPeriod_durationInDays = columns[17].replace("'","")
            enquiryPeriod_startDate = columns[18].replace("'","")[:10]
            enquiryPeriod_endDate = columns[19].replace("'","")[:10]
            enquiryPeriod_maxExtentDate = columns[20].replace("'","")[:10]
            enquiryPeriod_durationInDays = columns[21].replace("'","")
            hasEnquiries = columns[22].replace("'","")
            eligibilityCriteria = columns[23].replace("'","")
            awardPeriod_startDate = columns[24].replace("'","")[:10]
            awardPeriod_endDate = columns[25].replace("'","")[:10]
            awardPeriod_maxExtentDate = columns[26].replace("'","")[:10]
